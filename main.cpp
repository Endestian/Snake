...
int main()
{
    //perulangan kalo kena pager
    mulai:
    bool GameIsRunning=true;
    int GameSpeed=80; //kecepatan
    short dire[2]={0,1};
    vector<snakeBlock> snake;
    snakeInit(1,1,snake); //posisi awal snake
    drawMap(snake);
    while (GameIsRunning)
    {
        if (GetAsyncKeyState(VK_UP))//button
        {
            if (dire[1]==0)
            {
                dire[1]=-1;
...
// membuat NPC uler untuk adu kecepatan

// buat makanan bergerak
